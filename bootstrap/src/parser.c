...
static FRX_NO_DISCARD AST* __parser_parse_expression(Parser* parser)
{
    FRX_ASSERT(parser != NULL);
    ...
    if(parser_current_token(parser)->type == FRX_TOKEN_TYPE_LEFT_PARANTHESIS)
    {
        ...
        primary = parser_parse_primary_expression(parser);
    ...
    ASTBinaryExpression* binary_expression = arena_alloc(&parser->arena, sizeof(ASTBinaryExpression));
    binary_expression->left = primary;
    ...
    AST* expression = arena_alloc(&parser->arena, sizeof(AST));
    expression->type = FRX_AST_TYPE_BINARY_EXPRESSION;
    expression->node = binary_expression;

    if(binary_expression->right->type == FRX_AST_TYPE_BINARY_EXPRESSION
        && parser_get_precedence(binary_expression->type) <= parser_get_precedence(binary_expression->right->type))
    {
        ASTBinaryExpression* right = binary_expression->right->node;
        ASTType temp_type = binary_expression->type;
        binary_expression->type = right->type;
        right->type = temp_type;

        binary_expression->right = right->right;
        right->right = right->left;
        right->left = binary_expression->left;

        AST* wrapper = arena_alloc(&parser->arena, sizeof(AST));
        wrapper->type = FRX_AST_TYPE_BINARY_EXPRESSION;
        wrapper->node = right;
        binary_expression->left = wrapper;
    }
