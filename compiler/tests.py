...
Every test is part of one of two categories: 'fail' or 'pass'.

Category 'fail': A test that is meant to fail during compilation time.
Categiry 'pass': A test that is meant to be compiled *and* executed successfully.

Note: A successfull compilation/execution is indicated by an exit status of 0.

HOW TO CREATE A TEST:
Tests are defined in the tests directory inside the subdirectory named after their category.
Every project inside that subdirectory is treated as a test.
"""
...
COMPILER = "./bin/debug/furoxc" # Path to the compiler this script uses
...
def run_test(filepath, expected_result):
    ...
    if actual_result == expected_result:
        if expected_result == 0:
            # TODO: Use the below line when we no longer generate the executable in a local ./a.out file
            #exec_result = subprocess.run([temp_executable.name], capture_output=True, text=True)
            exec_result = subprocess.run(["./a.out"], capture_output=True, text=True)
    ...
    # TODO: Use the below lines when we no longer generate the executable in a local ./a.out file
    # if os.path.exists(temp_executable_name):
    #     os.remove(temp_executable_name)
    if os.path.exists("./a.out"):
        os.remove("./a.out")

def main():
    global PASS_COUNT, FAIL_COUNT
    valid_tests = [os.path.join(TEST_DIR, "pass", f) for f in os.listdir(os.path.join(TEST_DIR, "pass"))]
